...
По долгу службы мне часто приходится инициировать старт проектов. При старте проекта важным шагом является выбор средств обеспечения жизненного цикла
...
Так как мне надоедло тратить время на обосновние моих решения для членов команд и заказчиков я решил написать эту статью.
ф
...
Нам нужно решить какие именно инструменты мы будем использовать и где они будут размещаться.

Сначала давате все таки определимся с а потом уже перейдем к рассмотрению вариантов выбора этих систем.
...
| Repos                   | Хранение исходных кодов и контроль версий, например реализация git.
...
| Test Load               | Система
| Monitoring              | Мониторинг решения и предиктивная аналитики
| Feedback                | Система сбора отзывов от клиентов.
| IDE                     | Среда разработки, желательно из коробки интегрирвоанная со всеми системами.

Перечень получается внушительным внезапно это не говря уже о возможных дополнительных системах специализированных например для на создание скетчей и т.д.
